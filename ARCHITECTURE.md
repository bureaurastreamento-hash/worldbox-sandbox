...
- **`village/village.js`** — dados/factory da vila (estoque, população, território); `village.clanId` é atribuído por `clan/clan.js:addVillage`. `village.specialization` (`'food'` | `'wood'`) é passado na criação (`main.js`, sorteio sempre complementar entre as duas vilas) — toda vila tem `stock`/`capacity`/`demand` dos dois recursos, especializada ou não, só a produção (via `gather.js`/`gatherWood.js`) é que é exclusiva.
- **`village/stock.js`** — estoque comunitário e cálculo de demanda, genérico por chave de recurso (`Object.keys(village.capacity)`); é o valor que `gather.js`/`gatherWood.js` leem para enviesar o score, igual para todos os moradores, e que `trade.js` lê pra achar sobra/déficit.
