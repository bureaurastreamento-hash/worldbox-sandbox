...
Status: fatias 1-11 implementadas, mais especialização de vila, diplomacia dinâmica (com ataque ofensivo/saque, §7), evolução da civilização (minério + construção + papéis visuais, §8) e pesca/papel de guerreiro/animação de morte/arte de minério e água (§9) além do roteiro original — mais decoração do mapa com arte real (§5 item 12) e fome individual ligada ao estoque da vila (§6) numa sessão posterior. Numa sessão mais recente ainda, prioridade virou exclusivamente visual: reorganização completa de `assets/sprites/` (terreno água/grama/areia/floresta/montanha, ícones de minério, decoração árvore/planta/casa, personagem base e guerreiros Cavaleiro/Orc — ver §9 nota no topo, `STATUS.md` e `ROADMAP.md` §1.11/§2.4), encerrada por decisão do usuário com Elfo como única pendência conhecida de arte. Ver `STATUS.md` na raiz do projeto para o estado detalhado por sistema e os próximos passos concretos da sessão mais recente.
