...
- **Modelo de dados** (`predator/predator.js`): `Predator` é deliberadamente mais simples que `Agent` — sem needs/perception/memory/utility AI completo. Tem `health`/`maxHealth`, `state` (FSM, não score contínuo), `targetAgentId`, `spawnAnchor` (ponto de nascimento, usado como âncora do leash). 4 espécies (bear/wolf/snake/beatle), stats diferentes por espécie (`PREDATOR_SPECIES_STATS`) — beatle é o único à distância (o pack de origem tinha um projétil de baba, `Beatle_Slime_Shot`), os outros 3 são corpo a corpo.
...
- **Spawn** (`predator/predator.js:spawnPredators`): 24 no mundo (`PREDATOR_COUNT_PER_SPECIES=6`, 4 espécies), longe de qualquer vila (`PREDATOR_MIN_DISTANCE_FROM_VILLAGE_TILES=15` tiles), só em tile de grama/floresta. Raro de propósito — é pra ser um encontro perigoso pontual, não uma ameaça constante. Usa uma rng própria (`${seed}-predators`), mesmo padrão de `world/decorations.js` pra não desviar a sequência de `world.rng` (gameplay).
- **Arte**: substituiu a versão puramente decorativa que a fauna (Urso/Lobo/Cobra/Besouro) tinha antes — mesmos arquivos de sprite reaproveitados, mais uma pose de ataque nova por espécie. `world/decorations.js`/`decorationRenderer.js` não têm mais tipo `'animal'` — os quatro bichos existem só como `Predator` agora, nunca os dois ao mesmo tempo.
