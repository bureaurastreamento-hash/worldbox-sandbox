...
- **Papéis visuais** — **revertido numa sessão posterior, ver a nota no fim desta seção.** Como foi implementado na época: sprites por ação corrente (`render/agentRenderer.js`), não por clã/facção — qualquer vila tem os dois papéis. Decisões do usuário, perguntadas antes de implementar (nem toda ação tinha pose 1:1 óbvia na arte disponível):
...
**Poses por ação foram abandonadas (sessão posterior).** A leva de arte que sustentava essa seção — Camponês com pose dedicada de cortar árvore, minerar, construir, pescar e levar tronco — foi apagada pelo usuário por qualidade, e **nenhum pack disponível desde então tem pose de trabalho pra personagem humano**. Não é uma decisão de design revertida por gosto: é uma dependência de arte que deixou de existir.

O que entrou no lugar, quando `render/sprites/` (SpriteManager) chegou: cada civil é um dos 32 personagens da grade RPG do pack novo, escolhido por hash do `agent.id` e fixo pra vida toda. Ou seja, o projeto **voltou** ao eixo que a seção acima diz ter abandonado (diversidade visual por indivíduo) e perdeu o que tinha ganho (a pose refletir a ação). A troca não foi de graça, mas a alternativa era manter o civil como um círculo geométrico.

Duas coisas atenuam a perda e vale registrar: as partículas de trabalho (faísca ao minerar, lasca ao cortar árvore) continuam, então "está trabalhando ali" ainda se lê sem a pose; e a grade RPG trouxe **direção real de caminhada** (4 direções), que a leva anterior nunca teve. O guerreiro designado, esse sim, ganhou animação completa (idle/walk/attack/hurt/death) das tiras do pack novo.

Se algum dia aparecer um pack com poses de trabalho no estilo certo, o caminho de volta é curto: é reintroduzir clipes nomeados no ator do civil, não reescrever o renderer.
