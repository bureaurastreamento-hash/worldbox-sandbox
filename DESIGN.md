...
**Testado ao vivo, mas não por tempo suficiente pra ter certeza:** ~9 minutos simulados a 4x, duas vilas checadas (uma agrícola, uma guerreira em guerra) — população estável nas duas, nenhuma morte, nenhum erro no console, estoque de comida se comportando de forma sensata. Não foi tempo suficiente pra ver o estoque inicial da guerreira se esgotar de verdade nem pra confirmar que uma morte por fome individual acontece do jeito esperado quando ele chega a zero — fica registrado como próximo passo de observação no `STATUS.md`.
...
**Não implementado ainda:** vilas com população zerada (extintas em combate/colapso, mas que continuam existindo como entidade com estoque) continuam participando normalmente da diplomacia como se tivessem gente — não há verificação de "vila sem população não decide nada". (Isso agora também vale pro saque: uma vila raidada até zerar população continua podendo ser alvo/saqueada normalmente.)
