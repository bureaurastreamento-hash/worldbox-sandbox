...
**Ritmo de mineração/construção — confirmado funcionando numa sessão ao vivo:** o teste anterior (~3min) não tinha pescado nenhuma descoberta de minério ainda. Numa sessão seguinte, por sorte uma vila nasceu literalmente encostada numa cadeia de montanha — cenário ideal pra testar o `PERCEPTION_RADIUS` maior. Selecionando um agente diretamente (`AÇÃO: minerando` no painel), confirmei que ele realmente escolheu minerar assim que um depósito ficou dentro do raio de percepção; poucos segundos depois, `stone` no estoque da vila passou de 0/50 pra 1/50 — o ciclo completo (descobrir → minerar → entregar) funcionou de ponta a ponta. Não prova que o ritmo está bom em todos os casos (vilas que nascem longe de montanha ainda dependem só da sorte de um agente vagar até lá), mas confirma que a correção ataca a causa certa e o sistema funciona quando o depósito está ao alcance. `STATUS.md` registrava que mineração/construção nunca decolavam de verdade — em ~1h simulada de teste, nenhuma casa completava em nenhuma vila. Causa provável: `agent/actions/mine.js` só considera um depósito depois que o agente já viu o tile de montanha na própria percepção (`PERCEPTION_RADIUS`, então 8 tiles), e `wander.js` só escolhe entre tiles já visíveis agora — sem viés de exploração, achar montanha por acaso é um passeio aleatório lento. Ajustado: `PERCEPTION_RADIUS` subiu de 8 para 12 (mais chance de topar com montanha explorando, sem mudar a lógica de ninguém), e `HOUSE_WOOD_COST`/`HOUSE_STONE_COST` desceram de 30/20 para 20/12 (uma vez que a pedra chegue, precisa de menos acúmulo pra completar uma casa). Testado ao vivo por ~3 minutos reais (4x): população seguiu saudável e madeira começou a entrar aos poucos numa vila madeireira, mas nenhum minério tinha sido descoberto ainda dentro dessa janela curta — não dá pra confirmar o efeito real sem uma sessão bem mais longa jogada pelo usuário (mesma limitação de tooling já registrada: automação não sustenta sessão longa de forma confiável).
