...
- **Arte**: substituiu a versão puramente decorativa que a fauna tinha antes. `world/decorations.js`/`decorationRenderer.js` não têm mais tipo `'animal'` — os bichos existem só como `Predator` agora, nunca os dois ao mesmo tempo.
