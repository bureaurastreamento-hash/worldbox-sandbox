...
Snapshot do fim da sessão que implementou as fatias 1-10 (ver `DESIGN.md` seção 5) mais uma leva de correções, atualizado na sessão seguinte após consertar o sprite quebrado (§7), adicionar decoração do mapa (§8), completar a fatia 11 — UI de observação (§9), resolver a lacuna de especialização de vila (§10), implementar diplomacia dinâmica entre clãs (§11), corrigir bugs reportados jogando — LOD/câmera/robustez do loop (§12), começar a evolução da civilização — minério universal (§13) e completar construção + investigar/corrigir uma espiral de extinção séria (§14). Fatias 1-11 completas, mais especialização de vila, diplomacia dinâmica e evolução da civilização (minério + construção) implementadas além do roteiro original. Link ao vivo: https://bureaurastreamento-hash.github.io/worldbox-sandbox/ (GitHub Pages, atualiza a cada push em `main`).
...
Todas as 11 fatias do `DESIGN.md` (seção 5) estão implementadas agora. Evolução da civilização (§8, `DESIGN.md`) tem minério e construção feitos (§13-14); papéis visuais são o próximo:

1. **Papéis visuais** (evolução, próximo — ver §13) — usuário confirmou: sprites de trabalhador/guerreiro por AÇÃO corrente (minerando, construindo, lutando, etc.), não por clã/facção — qualquer vila tem os dois papéis. Tem uma leva de arte nova ainda não integrada em `assets/Assets-testes-para-o-claude-testar/` (Orc/Elfo/Cavaleiro/Componês com poses de ação) — perguntar ao usuário antes de usar, já que é conteúdo novo do amigo dele.

2. **Ataque ofensivo/saque** (pedido explícito do usuário, parte da diplomacia dinâmica — ver §11) — guerra hoje pode ser declarada dinamicamente por desespero econômico, mas combate continua só reativo (defesa quando territórios se cruzam). Falta a ação de saque de verdade: agentes marcham deliberadamente até a vila inimiga (`village.raidTargetVillageId`, campo já reservado em `village.js`) e saqueiam estoque, reaproveitando `deliver.js` (genérico por `carryingType`) pro transporte de volta. Combate em rota emergiria sozinho do sistema de `fight`/`flee` já existente, sem plumbing extra.

3. **Trocar o placeholder geométrico da decoração pela arte real** quando ela existir — reaproveitar `world.decorations` (dados) e só reescrever `render/decorationRenderer.js` (ver §8), igual ao pipeline dos sprites de agente. A leva de arte nova (item 1) inclui sprites de decoração (água, arbustos, palmeira, pinheiro) — pode ser a mesma conversa.

4. **Ligar a fome individual do agente ao estoque da vila** (ver `DESIGN.md` §6, limite conhecido) — hoje `eat.js` sempre come direto do ambiente, então uma vila guerreira sem comida nunca faz seus agentes passarem fome de verdade a nível individual, só a nível institucional (reprodução travada, needs decaindo mais rápido em colapso — ver §11). Mudança maior, mexe no loop de sobrevivência de todo agente; o cenário de colapso dinâmico (§11) é onde essa fome individual realmente importaria.

5. **Considerar depois**: vilas com população zerada (extintas por guerra/colapso) continuam participando normalmente da diplomacia dinâmica como se tivessem gente (ver `DESIGN.md` §7, não implementado ainda) — não é crítico, mas é uma inconsistência observável numa sessão longa.

6. **Considerar depois**: recalibrar a frequência de troca guerra/paz da diplomacia dinâmica (§11) — funciona e não crasha, mas numa sessão de observação longa a postura entre alguns pares de clã pode alternar com uma cadência que parece volátil. Depende de sensação de jogo, não só de número — vale revisitar depois de assistir de verdade, não só simular.

7. **Acompanhar de perto a estabilidade populacional** (ver §14, investigação extensa) — o jogo estava indo à extinção total de forma consistente antes dos ajustes desta sessão (`AGENT_COUNT` 5→8 foi o mais decisivo, junto com `MAX_AGE`, `REPRO_COOLDOWN`, `REPRO_FOOD_DEMAND_MAX` e pesos de ação). Validado via simulação direta por mais de 1h simulada com população saudável — mas **não** validado ao vivo numa sessão de jogo real (só simulação direta, que bypassa o throttling de `requestAnimationFrame`). Se o usuário observar extinção jogando de verdade, é esperado que aconteça bem menos que antes, mas o risco não está formalmente descartado — vale reportar se acontecer, com quanto tempo de jogo.

8. **Velocidade de construção/mineração** — mesmo com população saudável, pedra acumula devagar (depende de descoberta por acaso de depósito) e nenhuma casa chegou a ser construída em ~1h simulada de teste. Funciona, mas pode ser lento demais pra ser satisfatório numa sessão de jogo real — vale observar jogando antes de decidir se precisa de mais ajuste (ex.: raio de percepção maior, ou vilas nascerem mais perto de montanha).
...
## 14. Construção + investigação séria de extinção populacional (feito nesta sessão)

Ao implementar `agent/actions/build.js` (construção de casas, ver `DESIGN.md` §8), rodei simulações longas de validação (padrão já usado nesta sessão) e a população foi à **extinção total** (0 de 32 agentes, todas as 4 vilas) em ~500-600 segundos simulados — de forma consistente, em múltiplos seeds diferentes. Isso disparou uma investigação bem mais extensa do que o esperado, porque o resultado inicial parecia contradizer os testes de sucesso da sessão anterior (diplomacia dinâmica, §11).

### O que foi implementado (build.js)

- `utils/constants.js`: `HOUSE_WOOD_COST` (30), `HOUSE_STONE_COST` (20), `HOUSE_POP_BONUS` (5 por casa), `BUILD_WORK_SECONDS` (15), `BUILD_SCORE_WEIGHT` (0.5).
- `village/village.js`: `getPopulationCap(village)` = `VILLAGE_POP_CAP + buildings.length * HOUSE_POP_BONUS` — usado tanto por `build.js` (pontuação, pressão populacional) quanto por `lifecycle.js` (gate de reprodução), sem duplicar a fórmula. `village.buildings: []` (campo já reservado desde a fatia de minério) agora é populado de verdade.
- `agent/agent.js`: `buildProgress: 0` novo — trabalho acumulado na construção corrente, persiste entre reconsiderações.
- `agent/actions/build.js` (novo): anda até o centro da vila, reconfere o estoque na chegada (evita duplo-gasto se outro agente começou primeiro entre a partida e a chegada), acumula `buildProgress` enquanto parado lá, completa e empurra a casa pra `village.buildings` ao atingir `BUILD_WORK_SECONDS`.
- `ui/inspector.js`: população agora mostra `(N casas)` junto com o teto efetivo.

### A investigação da extinção — o que foi descartado, por ordem

Testado com **simulação direta em lotes** (importando os módulos reais e chamando as funções do loop manualmente, com `dt` fixo — o mesmo método já usado e documentado nesta sessão pra contornar o throttling de `requestAnimationFrame` em aba de segundo plano), sempre recarregando a página antes de cada teste pra garantir um mundo novo (um erro meu no meio do caminho — reusar um mundo já extinto de teste anterior — quase me fez tirar uma conclusão errada; fica registrado como lição).

1. **Hipótese: `mine`/`build` competindo demais por tempo de agente.** Testado desativando o score das duas ações via monkey-patch em `ACTION_TYPES` (sem alterar arquivo) — população **ainda foi extinta** no mesmo padrão. Descartada como causa única, mas mantive `MINE_SCORE_WEIGHT` (0.35, menor que `GATHER_SCORE_WEIGHT`) como melhoria de balanceamento válida por conta própria — dados de ação mostraram `mine` sendo escolhido desproporcionalmente mais que `gather`/`gatherWood` em alguns momentos.
2. **Hipótese: chance de guerra inicial alta demais com 4 clãs** (6 pares possíveis, cada um com sorteio independente — a chance de existir pelo menos uma guerra no mundo subiu de ~15% com 2 clãs pra ~62% com 4). Reduzido `INITIAL_STANCE_WEIGHTS.war` de 0.15 pra 0.06 — melhoria válida por si (menos guerras simultâneas descontroladas), mas população **ainda foi extinta** depois dessa mudança sozinha.
3. **Diagnóstico direto de causa de morte** (instrumentando `checkDeath` pra logar se cada morte foi por fome, combate ou idade): **100% das mortes eram por idade pura** (`age >= MAX_AGE`, zero fome, zero combate) em todos os testes limpos. Isso apontou pra reprodução não conseguindo repor os fundadores dentro da janela de vida deles — não é um problema de comércio/economia por si só.
4. **Tentativa: alargar o jitter de idade dos fundadores** (0-60 → 0-100). Piorou: um jitter mais largo faz alguns fundadores nascerem mais PERTO do limite de idade (não só mais espalhados), antecipando as primeiras mortes pra dentro da janela crítica de bootstrap. Revertido pra um jitter pequeno (0-15), só o suficiente pra evitar mortes no mesmo tick exato — a causa raiz não era falta de espalhamento, era falta de fôlego total.
5. **Instrumentação de bloqueio de reprodução** (contando cada tentativa e por que falhou): de 64 tentativas em 300s, quase todas bloqueadas — 48% por `demand.food > REPRO_FOOD_DEMAND_MAX` (0.7), 22% por `village.inChaos`, 28% por falta de adultos elegíveis. **Insight importante**: `REPRO_FOOD_DEMAND_MAX` testa a saúde do *estoque institucional* de comida, mas a fome *individual* do agente vem direto do ambiente (`eat.js`, desacoplado do estoque da vila — ver `DESIGN.md` §6). Pra uma vila madeireira (nunca produz comida própria), a demanda de comida fica perto de 100% quase sempre, mesmo com os moradores bem alimentados de verdade — o gate estava bloqueando reprodução por um motivo que não reflete fome real na maioria das vilas.

### O que de fato ajudou (mudanças mantidas)

- `REPRO_FOOD_DEMAND_MAX` 0.7 → 0.9 (gate bem mais permissivo, coerente com o insight acima).
- `MAX_AGE` 200 → 300 (mais fôlego real antes do primeiro fundador morrer).
- `REPRO_COOLDOWN_MIN/MAX` 20-40 → 12-25 (mais ciclos de tentativa de reprodução na mesma janela de tempo).
- Essas três juntas subiram a taxa de sucesso de reprodução de quase zero pra ~31% das tentativas (26 sucessos em 84 tentativas numa janela de teste) — melhora real, mas **ainda não bastou sozinha**: população ainda declinava e zerava eventualmente em testes mais longos.
- **A mudança decisiva: `AGENT_COUNT` 5 → 8** (fundadores por vila, 20→32 no total). Só isso, sem mexer em mais nada, transformou o resultado: população **cresceu** de 32 pra ~65-70 e se manteve estável (15-17 por vila, todas as 4 vilas vivas, zero colapso) por mais de **1 hora simulada** (testado incrementalmente em lotes, checando o estado a cada ~1000s simulados pra evitar travar a aba do navegador com uma única chamada gigante — isso realmente travou a aba uma vez, tive que fechar e abrir de novo). Conclusão: 5 fundadores por vila era pouca gente pra cobrir sobrevivência pessoal + economia (2 recursos de especialização + 4 minérios universais) + reprodução ao mesmo tempo, principalmente com 4 vilas competindo por atenção da diplomacia dinâmica.

### Limite honesto desta investigação

Não cheguei a isolar uma **causa raiz única e determinística** com certeza total — é mais provável que fosse uma combinação de fatores (economia lenta pra bootstrapar com poucos braços, gate de reprodução excessivamente severo, MAX_AGE curto demais pra essa economia) que se reforçavam mutuamente. As mudanças acima foram validadas empiricamente (simulação direta, múltiplos seeds, mais de 1h simulada de estabilidade) mas **não foram testadas numa sessão de jogo real, ao vivo, por um período longo** — só simulação direta em lote. Crescimento populacional também foi bem rápido no início (32→69 em 200s) — vale conferir se isso não passa a estourar o teto de população de forma pouco natural numa sessão bem mais longa que os testes cobriram.
